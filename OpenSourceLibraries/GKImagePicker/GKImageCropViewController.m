...
- (void)_setupNavigationBar{
    ...
    NSLog(@"_setupNavigationBar");
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel 
                                                                                          target:self 
                                                                                          action:@selector(_actionCancel)];
    
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"선택", @"")
...
- (void)_setupUseButton{
    ...
    NSLog(@"setupUseButton");
    if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1) {
        self.useButton = [UIButton buttonWithType:UIButtonTypeCustom];
		
        [[self.useButton titleLabel] setFont:[UIFont boldSystemFontOfSize:17]];
        [[self.useButton titleLabel] setShadowOffset:CGSizeMake(0, -1)];
        [self.useButton setFrame:CGRectMake(0, 0, 58, 30)];
        [self.useButton setTitle:NSLocalizedString(@"선택",@"") forState:UIControlStateNormal];
...
- (void)_setupToolbar{
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
        NSLog(@"cropViewCon setupToolbar UIUserInterfaceIdiomPhone");
        ...
        if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1) {
            ...
            info.text = NSLocalizedString(@"이동 및 크기 조절", @"");
        }
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    NSLog(@"gkimagecrop viewWillAppear");
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    NSLog(@"gkimagecrop viewwillDisappear");
    [[UIApplication sharedApplication] setStatusBarHidden:NO];
