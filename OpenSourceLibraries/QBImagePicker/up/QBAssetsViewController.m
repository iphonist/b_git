...
{
    NSLog(@"setAssetCollection");
    _assetCollection = assetCollection;
    
    [self updateFetchRequest];
    [self.collectionView reloadData];
    [self.collectionView setContentOffset:CGPointMake(0,0)];//self.tableView.contentSize.height)];
}
...
- (void)updateFetchRequest
{
    ...
    if (self.assetCollection) {
        ...
        NSLog(@"self.fetchResult %@",self.fetchResult);
...
{
    self.self.navigationItem.rightBarButtonItem.enabled = [self isMinimumSelectionLimitFulfilled];
    NSLog(@"self.doneButton %@ %@",self.self.navigationItem.rightBarButtonItem,self.self.navigationItem.rightBarButtonItem.enabled?@"YES":@"NO");
}
