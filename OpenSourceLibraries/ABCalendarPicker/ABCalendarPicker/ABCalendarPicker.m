...
{
	if ([text isEqual:LEFT_ARROW_STRING]) {
		[button setImage:[self imageNamed:@"ArrowLeft"] forState:UIControlStateNormal];
	} else if ([text isEqual:RIGHT_ARROW_STRING]) {
		[button setImage:[self imageNamed:@"ArrowRight"] forState:UIControlStateNormal];
	}
...
- (void)leftDeepPress:(UILongPressGestureRecognizer*)sender
{
    ...
    if (sender.state == UIGestureRecognizerStateBegan)
        [self performSelector:@selector(leftDeepPress:) withObject:sender afterDelay:0.3];
}
...
- (void)rightDeepPress:(UILongPressGestureRecognizer*)sender
{
    ...
    [self changeStateTo:self.currentState fromState:self.currentState animation:ABCalendarPickerAnimationNone canDiffuse:1];
    if (sender.state == UIGestureRecognizerStateBegan)
        [self performSelector:@selector(rightDeepPress:) withObject:sender afterDelay:0.3];
}
...
- (void)longLeftDeepPress:(UILongPressGestureRecognizer*)sender
{
    ...
    if (sender.state == UIGestureRecognizerStateBegan)
        [self performSelector:@selector(longLeftDeepPress:) withObject:sender afterDelay:0.3];
}
...
- (void)longRightDeepPress:(UILongPressGestureRecognizer*)sender
{
    ...
    [self changeStateTo:self.currentState fromState:self.currentState animation:ABCalendarPickerAnimationNone canDiffuse:0];
    if (sender.state == UIGestureRecognizerStateBegan)
        [self performSelector:@selector(longRightDeepPress:) withObject:sender afterDelay:0.3];
}
...
- (void)updateColumnNamesForProvider:(id<ABCalendarPickerDateProviderProtocol>)provider andState:(ABCalendarPickerState)state
{
    ...
    for (int j = 0; j < columnsCount; j++)
    {
        ...
        UILabel * columnLabel = [[UILabel alloc] initWithFrame:CGRectMake(floor(j*buttonWidth),19+28+16,buttonWidth,12)];
		...
		if (j == 0 && (state == ABCalendarPickerStateWeekdays || state == ABCalendarPickerStateDays)) {
			columnLabel.textColor = [UIColor colorWithRed:0.8 green:0 blue:0 alpha:1.0];
		} else {
...
//        [self addSubview:self.titleButton];
    ...
    CGFloat titleWidth = 120;// (self.longLeftArrow.hidden) ? 250 : 180;
    NSLog(@"leftarrow.hidden? %@",self.leftArrow.hidden?@"YES":@"NO");
    self.titleLabel.frame = CGRectMake(self.mainTileView.center.x - titleWidth/2,
                                       (self.columnLabels.count == 0) ? 7 : 2,
                                       titleWidth, 19+28+16);
    self.titleButton.frame = CGRectMake(self.titleLabel.center.x - buttonWidth/2, 0, buttonWidth, 19+28+16);
...
- (void)updateArrowsForProvider:(id<ABCalendarPickerDateProviderProtocol>)provider andState:(ABCalendarPickerState)state
{
    NSLog(@"updateArrowsForProvider");
    ...
    if ([provider animationForPrev] == ABCalendarPickerAnimationScrollUp)
		[self.leftArrow setImage:[self imageNamed:@"ArrowLeft"] forState:UIControlStateNormal];
    ...
    if ([provider animationForNext] == ABCalendarPickerAnimationScrollDown)
        [self.rightArrow setImage:[self imageNamed:@"ArrowRight"] forState:UIControlStateNormal];
    ...
    if ([(id)provider respondsToSelector:@selector(animationForLongPrev)])
    {
        if ([provider animationForLongPrev] == ABCalendarPickerAnimationScrollUp)
            [self.longLeftArrow setImage:[self imageNamed:@"ArrowLeft"] forState:UIControlStateNormal];
        ...
        if ([provider animationForLongNext] == ABCalendarPickerAnimationScrollDown)
            [self.longRightArrow setImage:[self imageNamed:@"ArrowRight"] forState:UIControlStateNormal];
        else if ([provider animationForLongNext] == ABCalendarPickerAnimationScrollRight)
            [self.longRightArrow setImage:[self imageNamed:@"ArrowRight"] forState:UIControlStateNormal];
    }
    
	CGFloat arrowY = 0;
	if (state != ABCalendarPickerStateWeekdays && state != ABCalendarPickerStateDays) {
		arrowY = 6;
	}
    self.leftArrow.frame = CGRectMake((self.longLeftArrow.hidden ? 0 : 38), arrowY, 38, 38);
    ...
    self.rightArrow.frame = CGRectMake(self.bounds.size.width-38-(self.longRightArrow.hidden ? 0 : 38), arrowY, 38, 38);
    ...
    {
        self.longLeftArrow.frame = CGRectMake(0, arrowY, 38, 38);
    ...
    {
        self.longRightArrow.frame = CGRectMake(self.bounds.size.width-38, arrowY, 38, 38);
...
- (void)updateDotsForProvider:(id<ABCalendarPickerDateProviderProtocol>)provider
{
    ...
    for (int i = 0; i < self.controls.count; i++)
    {
        ...
        for (int j = 0; j < arr.count; j++)
        {
            ...
            BOOL isPast = NO;
			if ([buttonDate compare:self.selectedDate] == NSOrderedAscending) {
				isPast = YES;
			}
            [self.styleProvider calendarPicker:self postUpdateForCellView:control onControlState:controlState withEvents:eventsCount andState:self.currentState isPast:isPast];
        }
    }
...
- (void)updateButtonsForProvider:(id<ABCalendarPickerDateProviderProtocol>)provider andState:(ABCalendarPickerState)state
{
    ...
    for (int i = 0; i < rowsCount; i++)
    {
        ...
        for (int j = 0; j < columnsCount; j++)
        {
            ...
            UIControl * control = [self.styleProvider calendarPicker:self cellViewForTitle:label andState:state isSunday:j == 0 ? YES : NO];
...
//            NSLog(@"shift %f",shift);
    ...
    NSLog(@"nowTileView %@ self %@",self.mainTileView,self);
}
           ...
           canDiffuse:(NSInteger)canDiffuse
{
    ...
    CGFloat newFrameHeight = _titleLabel.frame.size.height + 12 + buttonHeight*rowsCount + 1;

    if (self.gradientBar == nil)
    {
        self.gradientBar = [[UIImageView alloc] initWithImage:[self.styleProvider patternImageForGradientBar]];
        self.gradientBar.frame = CGRectMake(0, 0, self.bounds.size.width, _titleLabel.frame.size.height + 12);
        [self addSubview:self.gradientBar];
    }
    
    if (self.mainTileView == nil)
    {
        self.mainTileView = [[UIView alloc] initWithFrame:CGRectMake(0,_titleLabel.frame.size.height + 12,self.frame.size.width,self.frame.size.height-(_titleLabel.frame.size.height + 12))];
        NSLog(@"mainTile %@ self %@",self.mainTileView,self);
        self.mainTileView.userInteractionEnabled = NO;
        self.mainTileView.clipsToBounds = YES;
        self.mainTileView.backgroundColor = [UIColor whiteColor];//[UIColor colorWithRed:164/255. green:167/255. blue:176/255. alpha:1.0];
    ...
    if (self.patternImage == nil)
        self.patternImage = [[self imageNamed:@"TileNormal_02"] resizableImageWithCapInsets:UIEdgeInsetsMake(1, 1, 1, 1)];
...
//    NSLog(@"self.nowTileView %@",self.nowTileView);
//    control.clipsToBounds = YES;
    // Updating all elements
    
    [self updateButtonsForProvider:provider andState:toState];
    [self updateColumnNamesForProvider:provider andState:toState];
    [self updateArrowsForProvider:provider andState:toState];
    ...
    if (animation == ABCalendarPickerAnimationNone)
    {
        ...
        self.frame = CGRectMake(self.frame.origin.x,
                                ...
                                newFrameHeight);
        self.mainTileView.frame = CGRectMake(0,(_titleLabel.frame.size.height + 12),self.frame.size.width,self.frame.size.height-(_titleLabel.frame.size.height + 12));
...
{
    self.backgroundColor = [UIColor colorWithRed:255/255. green:255/255. blue:255/255. alpha:1.0];
