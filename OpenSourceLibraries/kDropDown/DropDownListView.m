...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions xy:(CGPoint)point size:(CGSize)size isMultiple:(BOOL)isMultiple rowHeight:(int)rowHeight
{
    isMultipleSelection=isMultiple;
    float height = MIN(size.height, DROPDOWNVIEW_HEADER_HEIGHT+([aOptions count]-1)*rowHeight+44);
...
//        _kTitleText = [aTitle copy];
        self._kDropDownOption = [aOptions copy];
        ...
        _kTableView.separatorColor = RGB(237,237,237);
        _kTableView.separatorInset = UIEdgeInsetsZero;
        _kTableView.backgroundColor = [UIColor clearColor];
        _kTableView.dataSource = self;
        _kTableView.delegate = self;
        globalHeight = rowHeight;
        
        if ([_kTableView respondsToSelector:@selector(setSeparatorInset:)]) {
            [_kTableView setSeparatorInset:UIEdgeInsetsZero];
        }
        if ([_kTableView respondsToSelector:@selector(setLayoutMargins:)]) {
            [_kTableView setLayoutMargins:UIEdgeInsetsZero];
        }
...
{
    if(indexPath.row == [self._kDropDownOption count]-1)
        return 44;
    else
        return globalHeight;
}
...
-(void)Click_Done{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(DropDownListView:Datalist:)]) {
        ...
        for (int k=0; k<self.arryData.count; k++) {
            ...
            [arryResponceData addObject:[self._kDropDownOption objectAtIndex:path.row]];
...
{
    return [self._kDropDownOption count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [self._kDropDownOption objectAtIndex:row];
...
//                                  rect.size.width -  2 * (DROPDOWNVIEW_SCREENINSET + 10), 0);
