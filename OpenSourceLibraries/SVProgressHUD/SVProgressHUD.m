...
#pragma clang diagnostic ignored "-Wdeprecated"
                ...
                NSDictionary *attributes = @{NSFontAttributeName:self.statusLabel.font, NSParagraphStyleAttributeName:paragraphStyle};
                stringSize = [string boundingRectWithSize:CGSizeMake(200.0f, 300.0f) options:NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
...
+ (NSTimeInterval)displayDurationForString:(NSString*)string {
    NSLog(@"display %f",(float)string.length * 0.06 + 0.5);
    NSLog(@"display %f",MAX((float)string.length * 0.06 + 0.5, [self sharedView].minimumDismissTimeInterval));
