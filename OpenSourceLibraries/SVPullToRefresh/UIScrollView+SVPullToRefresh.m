...
- (void)resetScrollViewContentInsetWithAnimation:(BOOL)animate;
- (void)setScrollViewContentInsetForLoading;
- (void)setScrollViewContentInset:(UIEdgeInsets)insets animation:(BOOL)animate;
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        if (self.pullToRefreshView.isObserving) {
            ...
            [self.pullToRefreshView resetScrollViewContentInsetWithAnimation:YES];
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(@"업데이트하려면 아래로 당기세요.",),
                             NSLocalizedString(@"업데이트하려면 당겼다 놓으세요.",),
                             NSLocalizedString(@"업데이트 중...",),
                                nil];
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        CGSize titleSize = [self.titleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth,self.titleLabel.font.lineHeight) options:NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
   ...
   //                                             lineBreakMode:self.titleLabel.lineBreakMode];
        ...
        attributes = @{NSFontAttributeName:self.subtitleLabel.font, NSParagraphStyleAttributeName:paragraphStyle};
         CGSize subtitleSize = [self.subtitleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth,self.subtitleLabel.font.lineHeight) options:NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
...
//        CGFloat labelX = (self.bounds.size.width / 2) - (totalMaxWidth / 2) + leftViewWidth + margin;
		...
		self.arrow.frame = CGRectMake(self.leftMargin,
		...
		CGFloat labelX = CGRectGetMaxX(self.arrow.frame) + 10.0;
...
//        self.arrow.frame = CGRectMake(15.0,
//                                      (self.bounds.size.height / 2) - (self.arrow.bounds.size.height / 2),
//                                      self.arrow.bounds.size.width,
//                                      self.arrow.bounds.size.height);
//        self.activityIndicatorView.center = self.arrow.center;
...
- (void)resetScrollViewContentInsetWithAnimation:(BOOL)animate {
    ...
    [self setScrollViewContentInset:currentInsets animation:animate];
}
...
- (void)setScrollViewContentInsetForLoading {
    ...
    [self setScrollViewContentInset:currentInsets animation:YES];
}

- (void)setScrollViewContentInset:(UIEdgeInsets)contentInset animation:(BOOL)animate {
...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        ...
        _titleLabel.text = NSLocalizedString(@"업데이트하려면 아래로 당기세요.",);
        _titleLabel.font = [UIFont boldSystemFontOfSize:14];
        _titleLabel.backgroundColor = [UIColor clearColor];
        _titleLabel.textColor = textColor;
		_titleLabel.shadowOffset = CGSizeMake(0.0, 0.5);
		_titleLabel.shadowColor = [UIColor colorWithWhite:0.9 alpha:0.9];
...
- (void)setState:(SVPullToRefreshState)newState {
    ...
    switch (newState) {
        case SVPullToRefreshStateAll:
			[self resetScrollViewContentInsetWithAnimation:YES];
            break;
        case SVPullToRefreshStateStopped:
		{
			[self resetScrollViewContentInsetWithAnimation:NO];
            break;
		}
        case SVPullToRefreshStateTriggered:
            break;
