...
#define EXTRA_VERTICAL_MARGIN 30.0
...
- (void)layoutSubviews
{
    ...
    NSDictionary *attributes = @{NSFontAttributeName:[UIFont systemFontOfSize:TITLE_FONT_SIZE], NSParagraphStyleAttributeName:paragraphStyle};
    
    CGSize titleSize = [self.title boundingRectWithSize:constrainedSize options:NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;

 //   CGSize titleSize = [self.title [UIFont boldSystemFontOfSize:TITLE_FONT_SIZE] constrainedToSize:constrainedSize];
    ...
    if (self.message) {
        ...
        NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc]init];
        paragraphStyle.lineBreakMode = NSLineBreakByWordWrapping;
        NSDictionary *attributes = @{NSFontAttributeName:[UIFont systemFontOfSize:MESSAGE_FONT_SIZE], NSParagraphStyleAttributeName:paragraphStyle};
        
         messageSize = [self.message boundingRectWithSize:constrainedSize options:NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
 ...
 //       messageSize = [self.message sizeWithFont:[UIFont systemFontOfSize:MESSAGE_FONT_SIZE] constrainedToSize:constrainedSize];
    ...
    NSLog(@"self.position %d",(int)self.position);
    switch (self.position) {
        case OLGhostAlertViewPositionBottom:
        default:
            yPosition = screenRect.size.height - ceilf(totalHeight) - self.bottomMargin - EXTRA_VERTICAL_MARGIN;
            break;
            
        case OLGhostAlertViewPositionCenter:
            yPosition = ceilf((screenRect.size.height / 2) - (totalHeight / 2)) - EXTRA_VERTICAL_MARGIN;
            break;
        ...
        case OLGhostAlertViewPositionCustomKeyboardVisible:
            yPosition = screenRect.size.height - ceilf(totalHeight) - self.bottomMargin - EXTRA_VERTICAL_MARGIN - self.keyboardHeight + 70;
            break;
    }
