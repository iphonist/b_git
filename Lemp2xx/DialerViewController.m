...
//    [btnNavi release];
    ...
    button = [CustomUIKit buttonWithTitle:nil fontSize:0 fontColor:nil target:self selector:@selector(loadLocalNumber) frame:CGRectMake(0, 0, 46, 44) imageNamedBullet:nil imageNamedNormal:@"bt_addic.png" imageNamedPressed:nil];
    btnNavi = [[UIBarButtonItem alloc]initWithCustomView:button];
...
//    [button release];
    ...
    allView.frame = CGRectMake(0,viewY,320,self.view.frame.size.height-viewY);
...
- (void)settingDialScreenWithHeight:(float)h dialer:(BOOL)dialer view:(UIView *)aView{
    ...
    if(dialer){
        ...
        CGSize size = [labelDialNumber.text sizeWithAttributes:@{NSFontAttributeName:labelDialNumber.font}];
        
        labelDialName = [CustomUIKit labelWithText:@"" bold:NO fontSize:15 fontColor:RGB(28,83,121) frame:CGRectMake(20, size.height+15, 280, dialScreenView.frame.size.height-(size.height+15)) numberOfLines:1 alignText:NSTextAlignmentCenter];//[[CustomUIKit labelWithText:@"" fontSize:20 fontColor:RGB(28,83,121) frame:CGRectMake(20, topView.frame.size.height/2, 280, 20) numberOfLines:1 alignText:NSTextAlignmentCenter];
                ...
                else // 번호 누르는 부분
                {
                    ...
                    if(calling){
                        ...
                        if([labelNumber hasPrefix:@"02"])
                        {
                            ...
                            {
                                digit = [SharedAppDelegate.root getPureNumbers:digit];
