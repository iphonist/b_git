...
//    [memberView release];
    ...
    memberCountLabel = [CustomUIKit labelWithText:[NSString stringWithFormat:@"즐겨찾기 멤버 %d명",(int)[[ResourceLoader sharedInstance].favoriteList count]] fontSize:14 fontColor:[UIColor blackColor] frame:CGRectMake(5, 0, 120, memberView.frame.size.height) numberOfLines:1 alignText:NSTextAlignmentLeft];
...
//    [viewMemberButton release];
    ...
    searchView = [[UIView alloc]initWithFrame:CGRectMake(0,memberView.frame.size.height,320,45)];
...
//    [search release];
    ...
    myTable.frame = CGRectMake(0, searchView.frame.size.height + memberView.frame.size.height, 320, self.view.frame.size.height - searchView.frame.size.height - memberView.frame.size.height - viewY);
...
- (void)reloadCheck{
    ...
    if([[ResourceLoader sharedInstance].favoriteList count]>0){
        ...
        memberView.frame = CGRectMake(0, 0, 320, 45);
        UIButton *viewMemberButton;
        viewMemberButton = [CustomUIKit buttonWithTitle:nil fontSize:0 fontColor:nil target:self selector:@selector(viewMember:) frame:CGRectMake(0,0, memberView.frame.size.width, memberView.frame.size.height) imageNamedBullet:nil imageNamedNormal:nil imageNamedPressed:nil];
        [memberView addSubview:viewMemberButton];
//        [viewMemberButton release];
        ...
        memberView.frame = CGRectMake(0, 0, 320, 0);
        
    }
    
    
    searchView.frame = CGRectMake(0,memberView.frame.size.height,320,45);
    
    
	    myTable.frame = CGRectMake(0, searchView.frame.size.height + memberView.frame.size.height, 320, self.view.frame.size.height - searchView.frame.size.height - memberView.frame.size.height); // 네비(44px) + 상태바(20px)
 ...
 NSString *headerTitle = @"";
    ...
    if(searching)
        ...
        if(section == 1-1){
            ...
            headerTitle = [NSString stringWithFormat:@"내 부서: %@ (%d명)",mydic[@"team"],(int)[deptList count]];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    for(NSDictionary *dic in [[ResourceLoader sharedInstance] allContactList]){
        NSString *deptcode = dic[@"deptcode"];      if([deptcode isEqualToString:mydeptcode])
            [deptList addObject:dic];
//        else
//            [addRestList addObject:dic];
