...
@interface GreenRequestViewController ()

@end

@implementation GreenRequestViewController
...
- (id)init{
    ...
    if(self){
        ...
        UILabel *label;
        label = [CustomUIKit labelWithText:NSLocalizedString(@"new_request", @"new_request") fontSize:14 fontColor:[UIColor whiteColor] frame:CGRectMake(2, 2, writeButton.frame.size.width - 5, writeButton.frame.size.height - 5) numberOfLines:1 alignText:NSTextAlignmentCenter];
...
//                    [cellData release];
        ...
        //        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"타임라인을 받아오는 데 실패했습니다. 잠시 후 다시 시도해 주세요!" delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"ok")NSLocalizedString(@"ok", @"ok") otherButtonTitles:nil, nil];
        //        [alert show];
...
//    PostViewController *post = [[PostViewController alloc]initWithStyle:kRequest];//WithViewCon:self];
    //    post.title = [NSString stringWithFormat:@"%@",titleString];
    [SharedAppDelegate.root.home.post initData:kRequest];
    SharedAppDelegate.root.home.post.title = NSLocalizedString(@"new_delivery_request", @"new_delivery_request");
